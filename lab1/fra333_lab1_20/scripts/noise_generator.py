...
class NoiseGenerator(Node):
    ...
    def __init__(self):
        ...
        # self.client = self.create_client(SetNoise,'/set_noise')
        
        # additional attributes
        self.mean = 0.0
        self.variance = 1.0
        self.get_logger().info(f'Starting {self.get_namespace()}/{self.get_name()} with the default parameter. mean: {self.mean}, variance: {self.variance} ,self.rate: {self.rate}')
    ...
    def timer_callback(self):
        noise = Float64()
        noise.data = np.random.normal(self.mean, self.variance)
